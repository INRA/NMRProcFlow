...
ui_proc_buttons <- column(4, 
     ...
     bsButton("headerMode", icon=icon("angle-double-up"), label = "", style="primary", type="toggle", value=FALSE ),
     bsButton("imgTerm", icon=NULL, label = "MD", style="primary", type="action" ),
     bsTooltip("resizeView", "Enlarge or Shrink the image height", "bottom", options = list(container = "body")),
     bsTooltip("headerMode", "Enable / Disable the banner and the main tabs", "bottom", options = list(container = "body")),
     bsTooltip("imgTerm", "Choose the image quality", "bottom", options = list(container = "body")),
     tags$br(), tags$br(),
     tags$br(), tags$br(),
     conditionalPanel(condition="input.condProcPanels !== 'Data Export' && input.condProcPanels !== 'Samples'", 
